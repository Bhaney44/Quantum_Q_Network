...
# Boltzmann machines (BMs) are an expressive family of models allowing the representation of a probability distribution over a set of binary variables. 
# These are often used to learn a stochastic, generative model over inputs. An undirected graph is used to represent conditional dependencies between variables.

# It is convenient to partition the variables into two sets of variable 
    #visibles $\mathbf{v}$
    #hiddens $\mathbf{h}$. 

# Visibles usually correspond to variables of interest to the applications, such as the inputs and outputs of our addition problem. 
# The hiddens are additional variables to enable the encoding of more complex dependencies amongst visible variables.
...
# Popular BM architectures include:
    ...
    # Restricted Boltzmann machines (RBMs), 
        #which have no lateral edges within the visibles or hiddens. These are bipartite graphs.
...
# The probability distribution encoded by a BM can be represented by an energy function $E(\mathbf{v},\mathbf{h})$ defined over the visible, $\mathbf{v}$, and hidden, $\mathbf{h}$, variables. 
    #In an RBM the energy function is defined as
    # E(\mathbf{v},\mathbf{h}) = \mathbf{b}^T\mathbf{v} + \mathbf{c}^T\mathbf{h} + \mathbf{h}^T\mathbf{Wv},


    # where $\mathbf{b}$ and $\mathbf{c}$ are biases (weights) applied to the visible and hidden variables respectively, and $\mathbf{W}$ is the weight matrix connecting the visible and hidden layers.
    ...
    # Z = \sum_{\mathrm{v},\mathrm{h}} \exp\{-E(\mathbf{v},\mathbf{h})\}.
...
# Integer addition is a ubiquitous operation. For $n$-bit addition, two $n$-bit operands are added to produce an $(n+1)$-bit sum. 
...
# We consider a 3-bit adder. It means that there are 10 visibles: two inputs that each have three bits and one output of four bits.

# Placing the units on Chimera
    # From the problem definition, a 3-bit adder requires one $K4$ graph (half adder) and two $K5$ graphs (full adders). 
...
# Model dimensions.
num_visible = 10
num_hidden = [9, 5] # the hidden layer is a bipartite graph of 9 by 5
...
# We have created a Boltzmann machine. We recall that a semi-RBM has an energy $E$, given as

# E = \mathbf{b}^T\mathbf{v} + \mathbf{c}^T\mathbf{h} + \mathbf{h}^T\mathbf{W}\mathbf{v} +\mathbf{h_L}^T\mathbf{X}\mathbf{h_R} .
...
# The parameters $\theta = \{\mathbf{b}, \mathbf{c}, \mathbf{W}, \mathbf{X}\}$ are what is learned. 
# Upon construction these parameters were populated with random values. If we already know a set of good parameters, we could provide these to the `r` object.

# We explore the shapes of the parameter vectors below. If all has gone as planned, they should easily follow from the model dimensions specified on `BM` creation.
...
# 
# For now we simply sample from all the visibles, which means sampling the binary random variables $v_i$ that make up the set of visibles $\mathbf{v}$ from a distribution
# 
# $$
# p(\mathbf{v}) = p(v_0, v_1, ..., v_{n-1}),
# $$
# 
# where $n$ is the number of visibles. Note that the BM represents a model over all variables, including the hiddens, we can sample the hidden variables also. This is usually not required for the application in mind.
# 
...
# The call to [`BM.sample_clamped_model`](documentation/dwave_bm.rbm.bm_class.html#dwave_bm.rbm.bm_class.BM.sample_clamped_model) should have returned a number of samples distributed over the encoded probability distribution $p(\mathbf{v})$. Currently we do not expect much structure in the distribution, and we can calculate some statistics to confirm this.
# 
...
# For our untrained BM, we would expect no combination of variable assignments to be favoured over any other. In that case, and since these are binary variables, we expect a mean near 0.5 and a standard deviation near 0.5.
# 
# If the samples we received from the untrained BM `r` conform to this, all should be in order. Next, we will train the BM.
...
# 
# We can then import the `BM` class from the `rbm` module. This class encodes the graphical representation of the BM and houses the methods for training it. We set up an instance `r` of the structure we require.
# 
...
# 
# Here we load the model we designed earlier. The designed model is available in the DIMACS format, a common format for specifying graph problems. Then we place it on the hardware.
...
# We now have all the information we need to start the training process. Executing the next cell will take some time. The logger we hooked into the training procedure above will write out the trained model to the disk at the frequency selected (every 100 epochs seems like a good frequency).
# 
# The method `BM.train` returns a list containing the RMSE after each logging step.
# 
# <div class="alert alert-warning" role="alert" style="margin: 10px"> 
# __CAUTION__: Training BM on the D-Wave quantum computer may take some time. Plan the following runs carefully!
# </div>
...
# 
# Previously, we set up and trained an semi-RBM on data describing the 3-bit addition problem. We formed the model and seeded its parameters with a QUBO problem built from constraints describing addition at the bit level. If this procedure was successful, we should now have a model of the distribution over the data.
# 
...
# The training error evolution looks better than it did for the PCD-trained model. There is a clear improvement over time (epoch).

# Now that our BM is trained, we can sample from the model it represents. For example, we can sample the sum bits while clamping the input bits to a value of interest. This allows us to sample from a particular instance of the addition problem.
# 
# More specifically, we can sample from the distribution
# 
# $$
# p(s|a,b) = p(v_6,v_7,v_8,v_9|v_0,v_1,...,v_5)
# $$
# 
# where $s$ is represented by the last four visibles, $a$ is represented by the first three visibles and $b$ is represented by the next three visibles.
# 
...
def sample_clamped_inputs(  r,              # The BM to sample from
                            a = [1, 0, 1],  # a = 5 by default
                            b = [0, 1, 1],  # b = 3 by default
                          ):
...
# If all has gone well, we should have achieved a reasonably high hit rate above. Unlike in the previous example, this indicates a narrow mode around the correct answer in the conditional distribution p(s|a,b)p(s|a,b).
# 
...
# There is, again, a narrow mode around the correct answer. Things are looking considerably better than with the PCD-trained model.
# 
...
# With limited number of epochs in the previous training, the distribution also started clustered widely around the correct answer, but did not evolve to convincingly favour the correct answer. In this case the evolution looks more like we expected before, with the distribution gradually tightening around the correct answer.
# 
...
# As in the single case examined above, the training seems to have been successful in this case. The correct answer was slightly favoured by the untrained (designed) model, but training allowed the model to adjust its weights to tighten up the distribution, until it peaks sharply at each correct answer.
# 
